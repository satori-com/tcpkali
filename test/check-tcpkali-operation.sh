...
check 5 "Total data sent:[ ]+149 bytes"     ${TCPKALI} -vv --ws
check 6 "Total data received:[ ]+278 bytes" ${TCPKALI} -vv --ws
check 7 "Total data sent:[ ]+158 bytes"     ${TCPKALI} -vv --ws --first-message ABC
check 8 "Total data received:[ ]+287 bytes" ${TCPKALI} -vv --ws -1 ABC
